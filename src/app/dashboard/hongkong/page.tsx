...
export default function HongKongPage() {
  const [etfs, setEtfs] = useState<ETFRecord[]>([]);
  const [timeSeriesData, setTimeSeriesData] = useState<TimeSeriesRecord[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([fetchETFs("HK"), fetchTimeSeries()])
      .then(([etfData, tsData]) => {
        setEtfs(etfData);
    ...
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="font-mono text-sm text-text-muted">Loading Hong Kong ETFs...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 font-mono text-sm text-text-muted">
              HKEX listed ETFs — {summary.etfCount} funds tracked
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <StatCard
          title="Total AUM"
          value={`HK$${(summary.totalAUM / 1000).toFixed(1)}B`}
      ...
      <div className="animate-fade-in-up delay-5 rounded-xl border border-border-primary bg-bg-card p-6">
        <h3 className="mb-1 text-sm font-medium text-text-primary">By Issuer</h3>
        <p className="mb-4 font-mono text-[10px] text-text-muted">
        ...
        <div className="flex gap-3">
          {issuers.map((issuer, i) => {
            ...
            return (
              <div key={issuer.name} className="flex-1 rounded-lg border border-border-primary bg-bg-tertiary p-4">
                <p className="font-body text-sm font-medium text-text-primary">{issuer.name}</p>
                <p className="mt-1 font-mono text-lg text-accent-amber">{share.toFixed(1)}%</p>
                ...
                <p className="mt-1 font-mono text-[10px] text-text-muted">
                  HK${(issuer.value / 1000).toFixed(0)}B
                </p>
              </div>
            );
          })}
      ...
      <div className="animate-fade-in-up delay-6 rounded-xl border border-border-primary bg-bg-card p-6">
        <h3 className="mb-1 text-sm font-medium text-text-primary">All Hong Kong ETFs</h3>
        <p className="mb-4 font-mono text-[10px] text-text-muted">Complete fund flow data</p>
        <ETFTable data={etfs} currencySymbol="HK$" />
      </div>
    </div>
  );
}
